    ...
    <div className="flex-1 mt-10 h-px border-t border-dashed border-border relative">
        <div className="absolute left-1/2 -top-4 group">
            <Button
                variant="ghost"
                size="icon"
                className="h-7 w-7 rounded-full bg-background border-2 border-dashed border-border opacity-0 group-hover:opacity-100 transition-opacity hover:bg-muted"
                >
...
const TriggerNodeContent = ({ config, onConfigChange }: { config: any, onConfigChange: (newConfig: any) => void }) => (
    <>
      <div className="grid grid-cols-4 items-center gap-4">
        ...
        <Input id="api-key" value={config.apiKey || ''} onChange={(e) => onConfigChange({ ...config, apiKey: e.target.value })} className="col-span-3" />
      </div>
      <div className="grid grid-cols-4 items-center gap-4">
        ...
        prevNodes.map(node => {
          if (node.id === editingNode.id) {
            ...
            if (node.id === 'trigger' && draftConfig.interval) {
              ...
              const artifactName = draftConfig.targetArtifact === 'spreadsheet-sales-data' 
                ? '@spreadsheet-sales-data' 
                : '@chart-revenue-by-region';
              newDescription = `Updates ${artifactName}`;
            }
        ...
        <div className="flex items-start justify-center w-full min-w-max gap-2">
          {nodes.map((node, index) => (
            <React.Fragment key={node.id}>
                <div 
                    className="flex flex-col items-center gap-2 text-center cursor-pointer group w-32"
                    onClick={() => handleOpenDialog(node)}
                >
                    <div className="w-20 h-20 rounded-xl bg-background border flex items-center justify-center shadow-sm group-hover:shadow-md group-hover:border-primary transition-all duration-200">
                    {node.icon}
                    </div>
                    <div>
